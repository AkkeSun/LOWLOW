...
public class TokenValidation {
    ...
    public ErrorDto duplicated(TokenDto token) {
        ...
        if(byRefreshToken.isEmpty())
            return ErrorDto.builder().errCode("wrongRefreshToken").errMsg("Refresh Token 이 올바르지 않습니다").build();

        // STEP 2 : refresh token 이 만료되었는지 확인
        else if(!jwtTokenProvider.validateTokenExceptExpiration(token.getRefreshToken()))
            return ErrorDto.builder().errCode("wrongRefreshToken").errMsg("Refresh Token 이 만료되었습니다").build();
